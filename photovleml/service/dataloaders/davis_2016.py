...
class DAVIS2016(Dataset):
    ...
    def __init__(
        ...
        if self.seq_name is None:
            ...
            # natsort.natsorted(ll, reverse=True)
            # Initialize the per sequence images for online training
            names_img = np.sort(natsort.natsorted(os.listdir(os.path.join(db_root_dir, 'JPEGImages/', str(seq_name))), reverse=False))
            ...
            labels = [os.path.join('Annotations/', str(seq_name), name_label[0])]
        ...
        self.img_list = natsort.natsorted(img_list, reverse=False)
        self.labels = labels
